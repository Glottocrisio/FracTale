...
def annotate_tale(tale_text, lda_model, dictionary):
    ...
    # Post-processing: ensure diversity and coherence of annotations
    ...
    for i in range(1, len(annotations) - 1):
        if annotations[i] == annotations[i-1] == annotations[i+1] == most_common:
            # If three consecutive annotations are the same and it's the most common,
            # try to diversify by choosing the second-best function
...
def annotate_episode_second_best(episode_text, lang, lda_model, dictionary, exclude):
    # Similar to annotate_episode, but returns the second-best function
