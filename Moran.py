...
def moran_i(text, max_distance=50):
    ...
    # Calculate Moran's I for different distances
    moran_values = []
...
def moran_i_clauses(text, max_distance=50):
...
def moran_i_sentences(text, max_distance=50):
    sentences = sent_tokenize(text)
...
def plot_moran_sentences(distances, moran_values, title):
    plt.figure(figsize=(10, 6))
    plt.plot(distances, moran_values, 'b-')
    plt.xlabel('Distance')
    plt.ylabel("Moran's I")
...
def moran_i_episodes(text, max_distance=50):
    episodes = text.split('\n')
...
corpora = ['grimm_tales_en', 'grimm_tales_de', 'grimm_tales_it', 'grimm_tales_es']
...
for corpus in corpora:
    ...
    for tale in tales:
        ...
        distances_clauses, moran_values = moran_i_clauses(tale)
        plot_moran_clauses(distances_clauses, moran_values)
        ...
        with open("moran_output.csv", 'a', newline='', encoding='utf-8') as csvfile:
